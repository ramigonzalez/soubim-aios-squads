...
### New Endpoints
- [x] `GET /api/projects/{id}/items` — List project items with full filter support
  - [x] Query params: `item_type`, `source_type`, `discipline`, `is_milestone`, `date_from`, `date_to`, `search`, `sort_by`, `sort_order`, `limit`, `offset`
  - [x] Multiple values for `item_type`, `source_type`, `discipline` (comma-separated)
  - [x] `discipline` filter uses JSONB `@>` operator against `affected_disciplines`
  - [x] `search` searches across `statement`, `who`, `why` fields
  - [x] Response includes `total`, `limit`, `offset`, `facets` (counts by item_type, source_type, discipline)
- [x] `GET /api/projects/{id}/items/{item_id}` — Get single item detail
  - [x] Returns full item including structured `impacts`, `consensus`, `source_excerpt`
  - [x] Includes source information (title, type, occurred_at) via JOIN
  - [x] Includes `affected_disciplines` as array of discipline strings
- [x] `POST /api/projects/{id}/items` — Create manual input item
  - [x] Required: `statement`, `item_type`, `who`, `affected_disciplines[]`
  - [x] Optional: `why`, `causation`, `owner` (action_items), `is_done`, context fields
  - [x] Auto-sets `source_type='manual_input'`
  - [x] Generates vector embedding on creation
  - [x] Returns created item with ID
- [x] `PATCH /api/projects/{id}/items/{item_id}` — Update item
  - [x] Supports: `is_milestone` toggle, `is_done` toggle, `statement` edit
  - [x] Admin-only for `is_milestone` toggle
  - [x] Returns updated item
- [x] `GET /api/projects/{id}/milestones` — List milestone-only items
  - [x] Equivalent to `/items?is_milestone=true`
  - [x] Supports same filter params as `/items`

### Backward-Compatible Endpoint
- [x] `GET /api/projects/{id}/decisions` — Preserved from V1
  - [x] Internally queries `project_items WHERE item_type='decision'`
  - [x] Response key: `"decisions"` (not `"items"`)
  ...
  - [x] Existing V1 query params still work: `discipline`, `meeting_type`, `date_from`, `date_to`, `search`, `confidence_min`, `has_anomalies`
  - [x] Response includes `Deprecation: true` header

### Pydantic Models
- [x] `ProjectItemResponse` — Full item response model
- [x] `ProjectItemCreate` — Manual input creation model
- [x] `ProjectItemUpdate` — Partial update model
- [x] `ProjectItemListResponse` — Paginated list with facets
- [x] `ImpactsSchema` — Structured impacts: `cost_impact`, `timeline_impact`, `scope_impact`, `risk_level`, `affected_areas[]`
- [x] `ConsensusSchema` — Flat map: `{discipline: {status: "AGREE"|"DISAGREE"|"ABSTAIN", notes: str?}}`
- [x] `DecisionResponse` — V1-compatible response for `/decisions` endpoint
- [x] All models have proper validation (item_type enum, discipline enum, etc.)

### Auth & Access Control
- [x] All endpoints require JWT authentication
- [x] `is_milestone` toggle restricted to admin/director role
- [x] Project-level access control: user must be member of project
- [x] Manual item creation: any project member
