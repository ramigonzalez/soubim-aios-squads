...
- [ ] **Create `decision-log-backend/app/prompts/extract_email.md`**
- [ ] Prompt adapted for email content structure (vs. meeting transcript structure)
- [ ] Instructs LLM to:
  - [ ] Extract all 5 item types: idea, topic, decision, action_item, information
  - [ ] Identify `who` from email sender and referenced names
  - [ ] Map speakers to disciplines using `ProjectParticipant[]` roster
  - [ ] Populate `affected_disciplines[]` per item following inference guidelines from PRD
  - [ ] Set `owner` for action items when mentioned
  - [ ] Handle email-specific patterns:
    - [ ] "Please review..." → action_item
    - [ ] "We've decided..." → decision
    - [ ] "FYI..." → information
    - [ ] "What if..." → idea
    - [ ] "We need to discuss..." → topic
- [ ] Prompt explicitly instructs: ignore quoted replies (text after `>` markers or `---Original Message---`)
- [ ] Prompt receives: `email_body`, `email_subject`, `email_from`, `email_to`, `participants[]`

### Email Extraction Service
- [ ] **Create `decision-log-backend/app/services/email_extractor.py`**
- [ ] Loads prompt from `app/prompts/extract_email.md` at runtime
- [ ] Strips quoted replies from email body before extraction:
  ```python
  def strip_quoted_replies(body: str) -> str:
      ...
      return '\n'.join(clean_lines)
  ```
- [ ] Calls Claude API with formatted prompt + cleaned email body
- [ ] Parses JSON response into `ProjectItem` records
- [ ] Each extracted item has:
  - [ ] `source_type='email'`
  - [ ] `source_id` linked to the email `Source` record
  - [ ] `item_type` classified by LLM
  - [ ] `affected_disciplines[]` inferred from participants and content
  - [ ] `statement`, `who`, `timestamp` (email date as timestamp)
...
- [ ] Email extraction triggered when email source status changes to `approved` (via Story 7.1 approval flow)
- [ ] Background task execution (FastAPI `BackgroundTasks`)
- [ ] Source status updated to `processed` after successful extraction
- [ ] Error handling: if extraction fails, source stays `approved`, error logged to `Source.processing_error`
- [ ] Re-processing support: admin can re-trigger extraction for a processed email

### Thread Deduplication
- [ ] When processing an email from a thread:
  - [ ] Check if other emails from the same `email_thread_id` have already been processed
  - [ ] Strip quoted content to avoid extracting the same items from replies
  - [ ] Log warning if potential overlap detected
- [ ] Each email in a thread is processed independently (new items only from the new content)

### Extraction Accuracy
- [ ] Target: 85%+ accuracy on item type classification (measured against test corpus)
- [ ] Test corpus: at least 5 curated email samples with known expected items
...
- `decision-log-backend/tests/test_email_extractor.py` — Unit tests
...
**Modified Files:**
- `decision-log-backend/app/services/ingestion_processor.py` — Add email source handling
- `decision-log-backend/app/services/__init__.py` — Register email extractor
