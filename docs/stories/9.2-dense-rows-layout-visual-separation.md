...
**Review:** @ux-design-expert (Uma) — Dense Rows visual validation
...
- [x] Date format: UPPERCASE short month + day + year: `FEB 8, 2026` (Overline style)
- [x] Typography: `text-xs font-bold tracking-wide uppercase text-gray-500`
- [x] Sticky position: `sticky top-0 z-10 bg-white/95 backdrop-blur-sm`
- [x] Bottom border: `border-b border-gray-200`
- [x] Item count badge: total items for that date, `text-xs text-gray-400 ml-2`
- [x] Height: 32px (`py-2 px-4`)
- [x] Groups items by `occurred_at` or `created_at` date

### Layer 2: Source Groups (Accordion, Left Border)
- [x] Source group header shows: `SourceIcon` + source title + item count + metadata
- [x] **Meeting sources:** title, time, participant count, duration, item count, summary icon
  - [x] Left border: `border-l-2` colored by primary discipline (from participants)
  - [x] Format: `📹 Client Alignment - Electrical | 3 items | 45min | 👥 3`
- [x] **Email sources:** subject, from, item count
  - [x] Left border: `border-l-2 border-sky-400`
  - [x] Format: `📧 RE: Foundation specs updated | 1 item`
- [x] **Document sources:** file name, file type, item count
  - [x] Left border: `border-l-2 border-orange-400`
- [x] **Manual input items** render directly under date header (no source group wrapper)
- [x] Typography: `text-sm font-semibold text-gray-900`
- [x] Collapsible accordion: expanded if ≤5 items, collapsed if >5
- [x] Accordion chevron: `ChevronDown` rotating 180° on toggle
- [x] Height: 48px (`py-3 px-4`)
- [x] Transition: `transition-transform duration-200` on chevron

### Layer 3: Item Rows (Dense, Single-Line)
- [x] **Create `ProjectItemRow` molecule** replacing `DecisionRow` for V2
- [x] Row height: 32-40px (`py-2 px-4`)
- [x] Row anatomy (left to right):
  1. `ItemTypeBadge` (icon only, no label) — 14px icon
  2. Statement text — `text-sm font-medium text-gray-900 truncate` (takes remaining width)
  3. `DisciplineCircles` — first 3 + "+N" overflow (sm size)
  4. Who — `text-xs text-gray-500 truncate max-w-[80px]`
  5. Timestamp/date — `text-xs text-gray-400 whitespace-nowrap`
  6. Milestone star (admin only, visible on hover) — `Star` icon, filled if milestone
- [x] Spacing between columns: `gap-2`
- [x] Row hover: `hover:bg-blue-50/50` with `transition-colors duration-150`
- [x] Row click: opens DrilldownModal (Story 3.7)
- [x] Keyboard: `role="button"`, `tabIndex={0}`, Enter to open modal
- [x] Focus: `focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1`

### Visual Differentiation Between Layers
- [x] Layer separation via: UPPERCASE vs sentence case, indent depth, border-l color, background
- [x] Date headers: no indent, uppercase, sticky, `bg-white/95`
- [x] Source groups: slight indent (`pl-2`), `border-l-2` colored, `bg-white`
- [x] Item rows: deeper indent (`pl-6`), no border-l, `bg-transparent`
- [x] Clear visual parent-child: source group wraps its items (like V1's `bg-gray-50/80` inset from Story 3.13)

### Items Without Source Group
- [x] Manual input items and orphan items render directly under their date header
- [x] No accordion wrapper — just standalone item rows
- [x] Slight visual distinction: no left border, standard indent

### Performance
- [x] 50+ items render without visible lag
- [x] Sticky date headers work smoothly during scroll
- [x] Accordion expand/collapse is instant (no layout shift)
- [ ] Consider virtualization if >200 items (defer to testing)

### Responsive Design
- [x] Desktop (1024px+): full layout as described
- [x] Tablet (768px): tighter column widths, shorter truncation
- [x] Mobile (375px): hide discipline circles, show only icon + statement + who, taller rows (48px)
...
1. **Modify `src/components/organisms/Timeline.tsx`**
   ...
   - Group items by date → then by source within each date
   - Render: Date header → SourceGroupAccordion(s) → ProjectItemRow(s)
   - Sticky date headers
...
2. **Create `src/tests/components/SourceGroupAccordion.test.tsx`**
   ...
   - Shows item count
   - Expanded by default if ≤5 items
   - Collapsed by default if >5 items
...
```
Timeline (Organism — redesigned for V2)
├── Loading: Skeleton rows
├── Empty: "No project items yet" message
├── Error: Alert + retry
└── For each date group:
    ├── DateHeader (UPPERCASE, sticky)
    │   ├── "FEB 8, 2026"
    │   └── Item count badge
    ├── SourceGroupAccordion (Molecule) [repeating per source]
    │   ├── Header: SourceIcon + title + meta + chevron
    │   ├── Left border (border-l-2, colored)
    │   └── Items area (collapsible):
    │       └── ProjectItemRow (Molecule) [repeating]
    │           ├── ItemTypeBadge (icon)
    │           ├── Statement (truncated)
    │           ├── DisciplineCircles
    │           ├── Who
    │           ├── Date
    │           └── Milestone star (admin, hover)
    └── Orphan Items (no source wrapper)
        └── ProjectItemRow [repeating]
```
...
|-------------|---------------|--------|
| `DecisionRow` | `ProjectItemRow` | New — replaces for V2 views |
| `MeetingGroup` | `SourceGroupAccordion` | New — generalized for all source types |
...
**Note:** `DecisionRow` and `MeetingGroup` are NOT deleted — they remain for V1 timeline (backward compat during transition). V2 Project History uses the new components.
...
**New Files:**
- `src/components/molecules/ProjectItemRow.tsx` — Dense item row molecule
- `src/components/molecules/SourceGroupAccordion.tsx` — Collapsible source group
- `src/tests/components/ProjectItemRow.test.tsx`
- `src/tests/components/SourceGroupAccordion.test.tsx`

**Modified Files:**
- `src/components/organisms/Timeline.tsx` — Major redesign to Dense Rows layout
- `src/lib/utils.ts` — Add `formatDenseDate()` utility
- `src/tests/components/Timeline.test.tsx` — Updated for Dense Rows structure
...
### Unit Tests
```
- ✅ ProjectItemRow renders all fields (badge, statement, circles, who, date)
- ✅ ProjectItemRow click and keyboard interaction
- ✅ ProjectItemRow milestone star admin visibility
- ✅ SourceGroupAccordion expand/collapse behavior
- ✅ SourceGroupAccordion left border color per source type
- ✅ Timeline Dense Rows: items grouped by date then source
- ✅ Timeline Dense Rows: UPPERCASE date headers
- ✅ Timeline Dense Rows: sticky headers
- ✅ Timeline Dense Rows: manual input items without source group
- ✅ Timeline Dense Rows: empty/loading/error states
...
| 2026-02-27 | Implemented — all components, tests, and redesigned Timeline |
