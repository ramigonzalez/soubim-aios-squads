...
### ItemTypeBadge Atom
- [ ] **Create `src/components/atoms/ItemTypeBadge.tsx`**
- [ ] Renders compact inline badge with icon + optional label
- [ ] Five item types with distinct colors and icons (from UX Guidelines 2.7):
  - [ ] Decision: `CheckCircle2`, `bg-green-100 text-green-700`
  - [ ] Topic: `MessageCircle`, `bg-amber-100 text-amber-700`
  - [ ] Action Item: `Target`, `bg-blue-100 text-blue-700`
  - [ ] Idea: `Lightbulb`, `bg-purple-100 text-purple-700`
  - [ ] Information: `Info`, `bg-slate-100 text-slate-700`
- [ ] Props: `type: ItemType`, `showLabel?: boolean` (default false for dense rows, true for expanded views)
- [ ] Icon size: `w-3.5 h-3.5` (14px)
- [ ] Badge styling: `inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium`
- [ ] Fallback for unknown type: gray badge with `HelpCircle` icon

### SourceIcon Atom
- [ ] **Create `src/components/atoms/SourceIcon.tsx`**
- [ ] Renders icon per source type with optional label and tooltip
- [ ] Four source types with distinct icons (from UX Guidelines 2.8):
  - [ ] Meeting: `Video`, `text-indigo-600`
  - [ ] Email: `Mail`, `text-sky-600`
  - [ ] Document: `FileText`, `text-orange-600`
  - [ ] Manual Input: `PenLine`, `text-gray-500`
- [ ] Props: `type: SourceType`, `showLabel?: boolean`, `size?: 'sm' | 'md'`
- [ ] Sizes: sm = `w-3.5 h-3.5`, md = `w-4 h-4`
- [ ] Tooltip on hover: source type label (CSS tooltip, no Radix dependency)
- [ ] Fallback for unknown source: `FileQuestion` icon in gray

### DisciplineCircle Atom
- [ ] **Create `src/components/atoms/DisciplineCircle.tsx`**
- [ ] Renders single-letter colored circle for a discipline
- [ ] Size: `w-6 h-6 rounded-full` (24px)
- [ ] Background: discipline hex color from canonical map
- [ ] Text: white, first letter of abbreviation, `text-xs font-bold`
- [ ] Props: `discipline: Discipline`, `isPrimary?: boolean`, `size?: 'sm' | 'md'`
- [ ] Primary indicator: `ring-2 ring-white ring-offset-1` on the first/primary discipline
- [ ] Sizes: sm = `w-5 h-5 text-[10px]`, md = `w-6 h-6 text-xs`
- [ ] Fallback: gray circle with "?" for unknown disciplines

### DisciplineCircles Group (Molecule Helper)
- [ ] **Create `src/components/atoms/DisciplineCircles.tsx`**
- [ ] Renders a stacked group of DisciplineCircle components
- [ ] Props: `disciplines: Discipline[]`, `max?: number` (default 3)
- [ ] Shows first `max` circles with overlapping stack: `-ml-1` (like avatar groups)
- [ ] First circle (primary) gets `isPrimary` ring indicator
- [ ] Overflow: `+N` counter badge (`text-xs text-gray-500 ml-1`) when disciplines > max
- [ ] Example: `[A][S][M] +2` for Architecture, Structural, MEP + 2 more

### Utility Functions
- [ ] **Add to `src/lib/utils.ts`:**
  - [ ] `getItemTypeColors(type: ItemType): { bg: string, text: string }` — returns Tailwind classes
  - [ ] `getItemTypeIcon(type: ItemType): LucideIcon` — returns Lucide icon component
  - [ ] `getItemTypeLabel(type: ItemType): string` — returns display label ("Action Item", not "action_item")
  - [ ] `getSourceTypeColors(type: SourceType): { bg: string, text: string }` — returns Tailwind classes
  - [ ] `getSourceTypeIcon(type: SourceType): LucideIcon` — returns Lucide icon component
  - [ ] `getSourceTypeLabel(type: SourceType): string` — returns display label
  - [ ] `getDisciplineCircleColor(discipline: Discipline): string` — returns hex color for circle bg
  - [ ] `getDisciplineInitial(discipline: Discipline): string` — returns single letter (A, S, M, etc.)
- [ ] All functions have fallback values for unknown/undefined inputs

### Design Consistency
- [ ] All components follow the 100/700 pattern from UX Guidelines (light bg, dark text)
- [ ] Color system matches PRD discipline enum (Section 2.2 of UX Guidelines)
- [ ] Components designed for both dense row context (inline, compact) and expanded context (modal, card)
- [ ] No new dependencies required — uses existing lucide-react and Tailwind
...
- `src/tests/components/ItemTypeBadge.test.tsx`
- `src/tests/components/SourceIcon.test.tsx`
- `src/tests/components/DisciplineCircle.test.tsx`
- `src/tests/components/DisciplineCircles.test.tsx`

**Modified Files:**
- `src/lib/utils.ts` — Add item type, source type, discipline circle utility functions
