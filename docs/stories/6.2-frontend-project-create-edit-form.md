...
### Routing & Navigation
- [ ] `/projects/new` route renders `ProjectForm` in create mode
- [ ] `/projects/:id/edit` route renders `ProjectForm` in edit mode (pre-populated)
- [ ] "Create Project" button on Projects page navigates to `/projects/new`
- [ ] Successful form submission redirects to project detail page
- [ ] "Cancel" button returns to previous page

### Project Details Section
- [ ] `title` — required text input, max 255 chars
- [ ] `description` — optional textarea, auto-resizing
- [ ] `project_type` — select dropdown with options: "Architecture Full", "Architecture Legal", "Custom"
- [ ] Form validation: title required, shows inline error if empty on submit

### Stage Schedule Builder
- [ ] Dynamic rows: each row has `stage_name` (text), `stage_from` (date), `stage_to` (date), remove button (X)
- [ ] "Add Stage" button appends empty row
- [ ] "Load Template" button:
  - [ ] Fetches templates from `GET /api/stage-templates`
  - [ ] Shows dropdown with template names
  - [ ] Selecting template populates stage rows with template stages
  - [ ] Template dates auto-calculated from today's date + `default_duration_days`
  - [ ] Replaces any existing stages (with confirmation if stages already defined)
- [ ] Date validation:
...
- [ ] Empty state: "No stages defined. Add stages or load a template."

### Participant Roster Section
- [ ] Dynamic rows: each row has `name` (text, required), `email` (email, optional), `discipline` (select from 15 disciplines, required), `role` (text, optional), remove button (X)
- [ ] "Add Participant" button appends empty row
- [ ] Discipline select shows all 15 discipline values with labels
- [ ] Email validation: valid email format when provided
- [ ] Duplicate email warning (within current roster)
- [ ] Empty state: "No participants added. Add team members to improve AI discipline inference."

### Form Submission
- [ ] Create mode: `POST /api/projects` with `{title, description, project_type, stages[], participants[]}`
- [ ] Edit mode: `PATCH /api/projects/{id}` with changed fields
  - [ ] Stage changes: replaces entire schedule via `POST /api/projects/{id}/stages`
  - [ ] Participant changes: diff and call add/update/remove endpoints
- [ ] Loading state on submit button
- [ ] Error handling: toast notification for API errors
- [ ] Success: redirect to project detail with success toast

### Responsive Design
- [ ] Mobile (375px): single-column layout, full-width inputs
- [ ] Tablet (768px): two-column layout for date pairs
- [ ] Desktop (1024px+): comfortable form width (max-w-3xl)
...
- [ ] Focus trap within form (standard browser behavior)
...
**New Files:**
- `src/components/organisms/ProjectForm.tsx` — Main form component
- `src/components/organisms/StageScheduleBuilder.tsx` — Stage builder
- `src/components/organisms/ParticipantRoster.tsx` — Participant roster
- `src/pages/ProjectCreate.tsx` — Create page
- `src/pages/ProjectEdit.tsx` — Edit page
- `src/hooks/useStageTemplates.ts` — Template fetcher
- `src/hooks/useProjectMutation.ts` — Project create/update mutations
- `src/tests/components/ProjectForm.test.tsx`

**Modified Files:**
- `src/App.tsx` or route config — Add new routes
