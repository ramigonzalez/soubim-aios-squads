...
- [ ] `ProjectCard` shows current stage name as a `StagePill` badge (e.g., "Estudo Preliminar")
  - [ ] `StagePill` styled: `text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium`
  - [ ] "Current" indicator: small dot before stage name (`w-1.5 h-1.5 rounded-full bg-blue-500`)
  - [ ] If no stages defined: show `text-gray-400 text-xs` "No stages"
- [ ] Project_type label shown: `text-xs text-gray-500 uppercase tracking-wide`
- [ ] Item count replaces decision count: "127 items" instead of "127 decisions"
- [ ] Card shows latest item date (not just latest decision date)

### Create Project Button
- [ ] "Create Project" button in page header, right-aligned
- [ ] Styled: `bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 flex items-center gap-2`
...
- [ ] When user has no projects:
  - [ ] Large icon: `FolderPlus` from lucide-react
  - [ ] Heading: "Create your first project"
  - [ ] Description: "Set up a project with a stage schedule to start capturing project items."
  - [ ] CTA button: "Create Project" → `/projects/new`
  - [ ] Centered, bordered/dashed container

### Backward Compatibility
- [ ] V1 projects (without stages) render correctly — show "No stages" instead of badge
- [ ] V1 projects show item_count from API (falls back to decision_count if needed)
...
- `src/tests/components/StagePill.test.tsx`

**Modified Files:**
- `src/components/common/ProjectCard.tsx` — Add stage, type, item count
- `src/pages/Projects.tsx` — Add create button, sort, empty state
- `src/tests/components/ProjectCard.test.tsx` — Updated tests
