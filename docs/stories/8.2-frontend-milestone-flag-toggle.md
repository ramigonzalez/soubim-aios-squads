...
### Star Toggle Visual
- [ ] `MilestoneStarToggle` molecule created with `Star` icon from lucide-react
- [ ] Active state: `fill-current text-yellow-500` (filled gold star)
- [ ] Inactive state: `text-gray-300` (outline star), visible only on hover in row context
- [ ] Already-flagged milestones always show the filled star (no hover required)
- [ ] Icon size: `w-4 h-4` in rows, `w-5 h-5` in DrilldownModal header
- [ ] `aria-label="Mark as milestone"` / `aria-label="Remove milestone"` based on state
- [ ] `role="button"` + `tabIndex={0}` + keyboard Enter/Space support

### Integration Points
- [ ] **Project History rows**: star appears right-aligned in `ProjectItemRow` (or existing `DecisionRow`)
  - [ ] Non-milestone items: star visible only on row hover (`group-hover:opacity-100 opacity-0 transition-opacity`)
  - [ ] Milestone items: star always visible (filled gold)
- [ ] **DrilldownModal**: star appears in modal header next to item statement
  - [ ] Click toggles milestone status from within modal
- [ ] **MilestoneTimeline**: milestone nodes show star on hover, allowing unmark from timeline view
  - [ ] Unmarking removes the item from the timeline on next re-render

### API Integration
- [ ] Calls `PATCH /api/projects/{id}/items/{item_id}` with `{ "is_milestone": true | false }`
- [ ] Optimistic update: star toggles immediately before API response
- [ ] On success: invalidates both `['projectItems', projectId]` and `['milestones', projectId]` React Query cache keys
- [ ] On error: reverts optimistic update + shows toast "Failed to update milestone. Changes reverted."

### Access Control
- [ ] Only `admin`/`director` role users see the star toggle
- [ ] Non-admin users see NO star at all (not disabled — completely hidden)
- [ ] Role check reads from Zustand auth store
...
**New Files:**
- `src/components/molecules/MilestoneStarToggle.tsx` — Star toggle molecule
- `src/hooks/useMilestoneToggle.ts` — Optimistic mutation hook
- `src/tests/components/MilestoneStarToggle.test.tsx`
- `src/tests/hooks/useMilestoneToggle.test.ts`

**Modified Files:**
- `src/components/molecules/DecisionRow.tsx` — Add star toggle (admin only)
- `src/components/organisms/DrilldownModal.tsx` — Add star in header
- `src/components/molecules/MilestoneNode.tsx` — Add unmark star
