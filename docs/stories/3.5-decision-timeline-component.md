...
- [x] Fetch decisions from `/api/projects/{id}/decisions` using React Query
- [x] Display decisions chronologically (default: newest → oldest)
- [x] React Query caching (5 min stale time) to avoid refetching
- [x] Timeline header shows total decision count

### Grouping Modes (v2 — Two Modes)
- [x] Two grouping modes via toggle: **"By Date"** (default) and **"By Discipline"**
- [x] **By Date mode:**
  - [x] Group decisions by `meeting_date` (fallback to `created_at`), sorted newest first
  - [x] Group header: Full date format "Friday, 7 February 2026" + decision count
  - [x] Blue circle node (`w-3 h-3 bg-blue-600 rounded-full`) at each group header
  - [x] Vertical timeline line connecting groups (`border-l-2 border-gray-200`)
  - [x] Decisions within group sorted by `timestamp` (meeting recording time)
- [x] **By Discipline mode:**
  - [x] Group decisions by `discipline` field
  - [x] Group header: Discipline name (e.g., "Architecture") + decision count
  - [x] Discipline-colored circle node at each group header (using discipline color tokens)
  - [x] Each row shows `meeting_date` inline (since discipline is the group header)
  - [x] Decisions within group sorted by date (newest first)
- [x] Grouping toggle state managed locally in component (`groupBy: 'date' | 'discipline'`)

### Decision Rows (v2 — Replaces Decision Cards)
- [x] `DecisionRow` component: compact 2-line row (~56-64px height)
- [x] **Row 1:** `decision_statement` (truncated, 1 line, `text-sm font-medium text-gray-900 truncate`) + discipline pill (abbreviated, right-aligned)
  - [x] Discipline pill: `text-xs px-2 py-0.5 rounded-full` using Tailwind `discipline-*` color tokens
  - [x] Abbreviations: Architecture→"Arch", MEP→"MEP", Structural→"Struct", Electrical→"Elec", Plumbing→"Plumb", Landscape→"Land"
- [x] **Row 2:** `who` (left, `text-xs text-gray-500`) + `timestamp` in `HH:MM:SS` format (right, `text-xs text-gray-500 font-mono`)
  - [x] User icon (`lucide-react User`, `w-3.5 h-3.5 text-gray-400`)
  - [x] Clock icon (`lucide-react Clock`, `w-3.5 h-3.5 text-gray-400`)
- [x] **NOT shown in row** (moved to DrilldownModal): description/why, consensus, impact, confidence, anomaly badges
- [x] Row styling:
  - [x] Background: `bg-white`
  - [x] Border: `border-b border-gray-100` (bottom border only)
  - [x] Border radius: `rounded-md`
  - [x] Spacing between rows: `space-y-2` (8px gap, per Gabriela's ~10px wireframe)
  - [x] Hover: `hover:bg-blue-50/50` + `cursor-pointer`
  - [x] Transition: `transition-colors duration-150`

### Interaction & Navigation (UX Insight 1: Transparency)
- [x] Click decision row → Open drill-down modal (Story 3.7, passed decision ID)
- [x] Hover state: subtle `bg-blue-50/50` highlight
- [x] Cursor pointer on decision rows
- [x] Focus visible for keyboard navigation (`ring-2 ring-blue-500 ring-offset-1`)
- [x] `role="button"` + `tabIndex={0}` + `onKeyDown` for Enter key support
- [x] No page reload when clicking row (modal opens inline)
...
- [x] Loading state: skeleton rows (3 groups × 3 thin pulsing bars each):
...
- [x] Empty state when no decisions:
...
- [x] Focus visible styles (`ring-2 ring-blue-500 ring-offset-1`)
