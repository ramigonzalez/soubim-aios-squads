...
### Dot Timeline Layout
- [ ] `MilestoneTimeline` organism renders within Project Detail page as a tab
- [ ] **Vertical line** runs top-to-bottom: `border-l-2 border-gray-300` centered in viewport
- [ ] **Stage dots (large)** — left side of line:
  - [ ] Each project stage renders as a large dot (`w-5 h-5 rounded-full`)
  - [ ] Stage info on left: `stage_name` (font-semibold) + `date range` (text-xs text-gray-500, "Mar 1 → Mar 31")
  - [ ] Stages flow chronologically top-to-bottom
  - [ ] Stages visually group/encapsulate milestones within their date range
  - [ ] Stage dot color: `bg-gray-400` default, `bg-blue-600` for current stage
- [ ] **Milestone dots (small)** — right side of line:
  - [ ] Each milestone renders as a smaller dot (`w-3 h-3 rounded-full bg-gray-500`)
  - [ ] Milestone content on right: `ItemTypeBadge`, `statement` (truncated), `SourceIcon`, `date`, `DisciplineCircle[]`
  - [ ] Milestones positioned within parent stage's date range
  - [ ] Connected to vertical line via thin horizontal connector (`border-t border-gray-300 w-4`)
- [ ] Stage dots and milestone dots visually distinct (size, color, weight)

### Current Stage & Today Marker
- [ ] **Current stage** distinguished: `bg-blue-600` dot, `ring-2 ring-blue-200`, "Current" label badge
- [ ] **"Today" marker**: horizontal dashed line across the timeline at today's date position
  - [ ] Label: "Today" in `text-xs text-blue-600 font-medium`
  - [ ] Line: `border-t-2 border-dashed border-blue-400`
  - [ ] Positioned proportionally between stage date ranges

### Data Integration
- [ ] Fetches stages from `GET /api/projects/{id}/stages`
- [ ] Fetches milestones from `GET /api/projects/{id}/milestones` (via `useMilestones` hook from Story 5.3)
- [ ] Groups milestones into their parent stage by date range
- [ ] Milestones outside any stage date range grouped under "Other" section at bottom
- [ ] Uses React Query with 5-minute stale time

### Milestone Item Display
- [ ] Each milestone row shows:
  - [ ] `ItemTypeBadge` — compact colored badge (from Story 9.1 or inline implementation)
  - [ ] `statement` — truncated to 1 line (`text-sm font-medium text-gray-900 truncate max-w-md`)
  - [ ] `SourceIcon` — small icon for source type (Video/Mail/FileText/PenLine)
  - [ ] `date` — formatted: "Feb 10, 2026" (`text-xs text-gray-500`)
  - [ ] `DisciplineCircle[]` — first 3 discipline circles + "+N" overflow
- [ ] Click milestone → opens DrilldownModal (Story 3.7, reused)

### States
- [ ] **Loading**: Skeleton with 3 stage dots and 5 milestone placeholders, `animate-pulse`
- [ ] **Empty**: "No milestones yet. Mark important items as milestones from the Project History."
  - [ ] Icon: `Star` from lucide-react
  - [ ] Dashed border container centered
- [ ] **No stages**: "Set up project stages in Project Settings to enable the timeline view."
- [ ] **Error**: Alert with retry button

### Responsive Design
- [ ] Desktop (1024px+): stages on left, milestones on right, comfortable spacing
- [ ] Tablet (768px): tighter spacing, shorter statement truncation
- [ ] Mobile (375px): vertical stack — stage header above its milestones, full-width rows

### Performance
- [ ] Loads in <2 seconds with up to 50 milestones
- [ ] No unnecessary re-renders (React.memo on milestone nodes)
- [ ] Lazy load milestone details on click (DrilldownModal)

### Accessibility
- [ ] Semantic HTML: `<nav aria-label="Milestone Timeline">`, `<section>` per stage
- [ ] Stage headers: `<h3>` with stage name
- [ ] Milestone items: `role="listitem"` within `role="list"`
- [ ] Keyboard navigation: Tab through milestones, Enter to open detail
- [ ] Focus visible: `ring-2 ring-blue-500 ring-offset-2`
- [ ] Screen reader: milestones announce type, statement, date, disciplines
...
- `src/components/molecules/MilestoneNode.tsx` — Milestone dot + content
- `src/components/molecules/StageNode.tsx` — Stage dot + label
- `src/tests/components/MilestoneTimeline.test.tsx`

**Modified Files:**
- `src/pages/ProjectDetail.tsx` — Add tab toggle for Milestone/History views
