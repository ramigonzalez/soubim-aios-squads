...
### Schema Migration
- [ ] `decisions` table renamed to `project_items` via `ALTER TABLE decisions RENAME TO project_items`
- [ ] All existing indexes renamed: `idx_decisions_*` → `idx_project_items_*` (6 indexes)
- [ ] New columns added to `project_items` with NOT NULL constraints where required:
  - [ ] `item_type VARCHAR(50) NOT NULL DEFAULT 'decision'` with CHECK constraint for 5 types
  - [ ] `source_type VARCHAR(50) NOT NULL DEFAULT 'meeting'` with CHECK constraint for 4 types
  - [ ] `is_milestone BOOLEAN NOT NULL DEFAULT false`
  - [ ] `is_done BOOLEAN NOT NULL DEFAULT false`
  - [ ] `affected_disciplines JSONB NOT NULL DEFAULT '[]'`
  - [ ] `owner VARCHAR(255)` (nullable — only for action_items)
  - [ ] `source_id` UUID FK → sources (nullable — populated in Phase 4 of migration)
  - [ ] `source_excerpt TEXT` (nullable)
- [ ] `statement` column added as alias/rename of `decision_statement` (keep both during transition)
- [ ] CHECK constraints added: `item_type IN ('idea','topic','decision','action_item','information')`, `source_type IN ('meeting','email','document','manual_input')`

### Data Preservation & Transformation
- [ ] Pre-migration validation query confirms all existing `discipline` values are in V2 enum
- [ ] V1 `discipline='interior'` records mapped to `'architecture'` (if any exist)
- [ ] `affected_disciplines` populated from single `discipline` field: `jsonb_build_array(discipline)`
- [ ] Existing `consensus` JSON transformed from V1 flat map `{"architecture":"AGREE"}` to V2 format `{"architecture":{"status":"AGREE","notes":null}}`
- [ ] Existing `impacts` JSON preserved as-is (V2 structured format applies only to new items)
- [ ] All existing decisions retain `item_type='decision'`, `source_type='meeting'`
- [ ] All existing decisions retain `is_milestone=false`, `is_done=false`
- [ ] Row count before and after migration matches exactly

### New Tables Created
- [ ] `sources` table with all columns per PRD Data Model section
- [ ] `project_participants` table with all columns per PRD
- [ ] Source-type-specific CHECK constraints on `sources` table (meeting requires meeting_type and webhook_id)

### Transcript → Source Migration
- [ ] Source records created from existing `transcripts` table data
- [ ] Each transcript becomes a Source with: `source_type='meeting'`, `title=meeting_title`, `raw_content=transcript_text`, `occurred_at=meeting_date`, `ingestion_status='processed'`, `meeting_type`, `participants`, `duration_minutes`, `webhook_id`
- [ ] `project_items.source_id` linked to corresponding Source record via transcript_id mapping
- [ ] `transcripts` table preserved as read-only archive (NOT dropped)
- [ ] `project_items.transcript_id` column preserved as nullable legacy FK

### Index Plan (15 indexes on new structures)
- [ ] `project_items` new indexes:
...
- [ ] Pre-migration backup script provided (pg_dump command)
- [ ] Down migration script works correctly **when no V2-specific data exists**
- [ ] Down migration documented as "development-only safety net — production rollback uses pre-migration backup"
- [ ] Migration tested against copy of production data (or seed data)

### Backward Compatibility
- [ ] Existing V1 queries against `project_items` (formerly `decisions`) return same results
- [ ] `decision_statement` column preserved (not dropped) for backward compatibility
- [ ] `discipline` column preserved (not dropped) for backward compatibility
- [ ] `transcript_id` column preserved as nullable FK
- [ ] All existing V1 tests pass after migration (with model import path updates)
- [ ] Vector search continues to function with migrated embeddings
...
**New Files:**
- `decision-log-backend/app/database/migrations/001_decisions_to_project_items.py` — Core table migration
- `decision-log-backend/app/database/migrations/002_add_sources_participants.py` — New tables + transcript linking
- `decision-log-backend/tests/unit/test_migration_v2.py` — Migration test suite
- `decision-log-backend/scripts/pre_migration_backup.sh` — Backup script

**Modified Files:**
- `decision-log-backend/app/database/models.py` — Rename Decision→ProjectItem, add Source, ProjectParticipant models, extend Project
- `decision-log-backend/app/api/routes/decisions.py` — Update imports (Decision→ProjectItem)
- `decision-log-backend/app/database/seed.py` — Update to use ProjectItem model name
- `decision-log-backend/tests/unit/test_database.py` — Update for renamed model/table
- `decision-log-backend/tests/conftest.py` — Update fixtures for new models
