...
### Filter Bar Layout
- [x] Horizontal filter bar rendered immediately above the Dot Timeline content
- [x] Two filter groups: "Source" and "Item Type"
- [x] Active filter count badge: `text-xs bg-blue-600 text-white rounded-full px-1.5 py-0.5` — e.g., "3"
- [x] "Clear filters" button: `text-xs text-blue-600 hover:underline` — resets all to unselected
- [x] Clear button hidden when no filters active

### Source Type Filter Chips
- [x] Four chips, each with icon + label:
  - [x] Meeting: `Video` icon, active: `bg-indigo-100 text-indigo-700`, inactive: `bg-gray-100 text-gray-600`
...
- [x] Chip styling: `inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full cursor-pointer transition-colors`
- [x] Toggle behavior: click activates/deactivates (multi-select — can have 0 or N active)

### Item Type Filter Chips
- [x] Five chips, each with icon + label:
...
- [x] Same chip styling and toggle behavior as source type chips

### Filter Logic
- [x] AND across dimensions: selecting "Meeting" + "Decision" shows only milestones from meetings that are decisions
- [x] OR within dimension: selecting "Meeting" + "Email" shows milestones from either source type
- [x] Empty filter = show all milestones (no filter active = unfiltered)
- [x] Client-side filtering of the already-fetched milestone list (no additional API calls)
- [x] Empty result state: "No milestones match your filters." + "Clear filters" link

### URL Persistence
- [x] Filter state encoded in URL search params: `?source=meeting,email&type=decision,action_item`
- [x] On page load: read URL params and restore filter state
- [x] Changing filters updates URL params without page reload (use `replaceState`)
- [x] Shareable: copying URL and opening it restores the exact filter state
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ Source: [📹 Meeting] [✉️ Email] [📄 Document] [✏️ Manual]                  │
│ Type:   [✓ Decision] [💬 Topic] [🎯 Action] [💡 Idea] [ℹ️ Info]  3 ╳Clear │
└─────────────────────────────────────────────────────────────────────────────┘
```

### AND/OR Logic

- **Within dimension (OR):** "Meeting" + "Email" → show milestones from EITHER meeting OR email
- **Across dimensions (AND):** "Meeting" + "Decision" → show milestones from meetings that are ALSO decisions
...
**New Files:**
- `src/components/molecules/MilestoneFilterBar.tsx` — Filter chip bar
- `src/hooks/useMilestoneFilters.ts` — URL-persisted filter state hook
- `src/tests/components/MilestoneFilterBar.test.tsx`
- `src/tests/hooks/useMilestoneFilters.test.ts`

**Modified Files:**
- `src/components/organisms/MilestoneTimeline.tsx` — Integrate filter bar + apply filtering
- `src/lib/utils.ts` — Add `filterMilestones()` utility
...
### Unit Tests
```
- ✅ All 4 source type chips render
- ✅ All 5 item type chips render
- ✅ Click toggles chip active/inactive
- ✅ Active chip applies correct color class
- ✅ Inactive chip applies neutral gray class
- ✅ Active count badge reflects correct number
- ✅ Clear all resets to zero active
- ✅ filterMilestones: no filters = all items returned
- ✅ filterMilestones: source filter restricts by source_type
- ✅ filterMilestones: item filter restricts by item_type
- ✅ filterMilestones: combined filters use AND across, OR within
- ✅ URL params set on filter change
- ✅ URL params read on initial load
- ✅ Empty filter result shows message
```
...
| 2026-02-28 | Implemented all tasks: MilestoneFilterBar, filterMilestones utility, useMilestoneFilters hook, MilestoneTimeline integration, tests (44 passing) |
