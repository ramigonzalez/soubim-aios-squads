...
- [x] `POST /api/projects` extended with: `title` (required), `description`, `project_type`, `stages[]`, `participants[]`
  - [x] Creates project + stage schedule + participants in one transaction
  - [x] Returns created project with stages and participant count
- [x] `PATCH /api/projects/{id}` updates: `title`, `description`, `project_type`, `actual_stage_id`
  - [x] `actual_stage_id` must reference an existing stage for this project
- [x] `DELETE /api/projects/{id}` archives project (sets `archived_at`, does NOT hard-delete)
- [x] `GET /api/projects/{id}` extended response includes: `stages[]`, `participant_count`, `current_stage`, `project_type`
- [x] `GET /api/projects` list includes: `current_stage_name` badge, `project_type`, `item_count` (replaces `decision_count`)

### Stage Schedule
- [x] `GET /api/projects/{id}/stages` — List stages ordered by `sort_order`
- [x] `POST /api/projects/{id}/stages` — Set stage schedule (replaces all stages)
  - [x] Accepts array of `{stage_name, stage_from, stage_to}`
  - [x] Auto-generates `sort_order` from array position
  - [x] Date validation: `stage_from < stage_to` for each stage
  - [x] Date validation: no overlapping date ranges between stages
  - [x] Date validation: stages sequential (stage N's `stage_to` ≤ stage N+1's `stage_from`)
- [x] `PATCH /api/projects/{id}/stages/{stage_id}` — Update single stage (name, dates)
  - [x] Re-validates date constraints after update
- [x] `actual_stage_id` updated via `PATCH /api/projects/{id}` (not on stage endpoint)

### Stage Templates
- [x] `GET /api/stage-templates` — List predefined templates
- [x] `stage_templates` table seeded with at least 2 templates:
  - [x] **Architecture Full:** Briefing (30d) → Levantamento (45d) → Estudo Preliminar (60d) → Anteprojeto (60d) → Projeto Legal (30d) → Projeto Executivo (90d) → Acompanhamento de Obra (180d)
  - [x] **Architecture Legal:** Briefing (15d) → Estudo Preliminar (30d) → Projeto Legal (45d) → Projeto Executivo (60d)
- [x] Template response includes: `id`, `project_type`, `template_name`, `stages[]` with `{stage_name, default_duration_days}`

### Project Participants
- [x] `GET /api/projects/{id}/participants` — List participants
- [x] `POST /api/projects/{id}/participants` — Add participant
  - [x] Required: `name`, `discipline`
  - [x] Optional: `email`, `role`
  - [x] Validates discipline against Discipline enum (15 values)
  - [x] Enforces unique email per project (when email provided)
  - [x] Enforces unique name per project (when no email provided)
- [x] `PATCH /api/projects/{id}/participants/{id}` — Update participant
- [x] `DELETE /api/projects/{id}/participants/{id}` — Remove participant
- [x] Participant count included in project detail response

### Database (Migration 003)
- [x] `project_stages` table created (if not already in 5.1)
- [x] `stage_templates` table created and seeded
- [x] `projects` table extended: `project_type VARCHAR(100)`, `actual_stage_id UUID FK → project_stages`
- [x] Indexes: `idx_stages_project`, `idx_stages_sort`

### Backward Compatibility
- [x] Existing project listing still works (V1 projects without stages show `null` current_stage)
- [x] `GET /api/projects` response shape extends, does not break V1 consumers
- [x] Projects without stages are valid (stages optional)
