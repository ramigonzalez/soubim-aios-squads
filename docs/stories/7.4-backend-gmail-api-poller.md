...
- [x] Google OAuth2 credentials loaded from environment variables (`GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` or `GMAIL_SERVICE_ACCOUNT_JSON`)
- [x] `GmailAuthService` handles token refresh automatically (access tokens expire after 1 hour)
- [x] Service account flow supported for server-to-server auth (no user interaction)
- [x] User consent OAuth2 flow documented as alternative in Dev Notes (for multi-tenant future)
- [x] Auth failure logs error and skips polling cycle — does NOT crash the background scheduler

### Polling Configuration
- [x] `GMAIL_POLL_INTERVAL_MINUTES` environment variable controls poll frequency (default: `30`)
- [x] `GMAIL_LABEL_FILTER` environment variable specifies Gmail label(s) to match (default: `""` — matches all unread)
- [x] `GMAIL_MAX_RESULTS_PER_POLL` environment variable caps emails fetched per cycle (default: `50`)
- [x] All three env vars added to `decision-log-backend/app/config.py` with proper defaults
- [x] Polling interval validated: minimum `5` minutes enforced (guards against API quota exhaustion)
- [x] Configuration logged at scheduler startup (interval, label filter, max results)

### Email Matching & Project Association
- [x] `EmailMatcherService` determines which project a polled email belongs to
- [x] Matching strategy: Gmail label name matches project slug or project-specific label (e.g., `project/skyline-tower`)
- [x] Fallback matching: email subject or sender domain matched against project metadata
- [x] Unmatched emails logged at `WARNING` level and skipped (not stored as Source records)
- [x] Matched project ID stored on Source record

### Source Record Creation
- [x] For each new (non-duplicate) email: one `Source` record created with:
  - [x] `source_type = 'email'`
  - [x] `ingestion_status = 'pending'`
  - [x] `raw_content` = decoded plain-text email body (HTML stripped)
  - [x] `title` = email subject line
  - [x] `occurred_at` = email sent date (parsed from Gmail `Date` header)
  - [x] `email_from` = sender address
  - [x] `email_to` = list of primary recipients (JSONB array)
  - [x] `email_cc` = list of CC recipients (JSONB array)
  - [x] `email_thread_id` = Gmail thread ID
  - [x] `project_id` = matched project UUID
- [x] Source record saved to `sources` table (from Story 5.1 migration)
- [x] Message ID stored in `webhook_id` field for deduplication lookup

### AI One-Line Summary
- [x] Claude 3.5 Sonnet called for each new email Source record to generate a one-line summary
- [x] Summary stored in `ai_summary` field on Source record
- [x] Summary prompt: `"Summarize this email in one sentence for a project manager: {email_body}"`
- [x] Summary capped at 150 characters; truncated with ellipsis if Claude returns more
- [x] AI summary failure does NOT block Source record creation — `ai_summary` left null on error
- [x] AI summary errors logged at `WARNING` level with email subject for traceability

### Deduplication
- [x] Before creating a Source record, check for existing record with same `email_thread_id` AND `webhook_id` (message ID)
- [x] If duplicate found: skip silently (log at `DEBUG` level)
- [x] Thread-level deduplication only (distinct messages in same thread ARE stored separately)
- [x] Deduplication check runs inside a database transaction to prevent race conditions

### Rate Limiting & Quota Management
- [x] Gmail API calls wrapped in exponential backoff retry logic
- [x] Retry schedule: wait `2^attempt` seconds (1s, 2s, 4s, 8s, 16s), max 5 retries
- [x] `googleapiclient.errors.HttpError` with status `429` (quota exceeded) triggers backoff
- [x] `googleapiclient.errors.HttpError` with status `500`/`503` (transient) triggers backoff
- [x] After max retries exhausted: log `ERROR` and skip current polling cycle
- [x] Total Gmail API calls per poll cycle logged for monitoring

### Background Scheduler
- [x] APScheduler `BackgroundScheduler` initialized in `decision-log-backend/app/scheduler.py`
- [x] Gmail polling job registered with `IntervalTrigger` using `GMAIL_POLL_INTERVAL_MINUTES`
- [x] Scheduler started on FastAPI `startup` event and shut down on `shutdown` event
- [x] Job runs are non-overlapping: if previous poll cycle still running, new trigger skipped
- [x] Scheduler state (next run time, last run time) accessible via `GET /api/admin/scheduler/status` (admin only)
