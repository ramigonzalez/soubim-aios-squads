...
### Routing & Navigation
- [x] `/projects/:id/items/new` route renders `ManualItemCreate` page
- [x] `ManualItemCreate` page renders the `ManualItemForm` organism
- [ ] "Add Item" button on Project History page navigates to `/projects/:id/items/new`
- [x] "Cancel" button returns to `/projects/:id/history` without saving

### Core Form Fields
- [x] `statement` — required textarea, placeholder "Describe the item in one or two sentences", no character limit enforced
- [x] `item_type` — required select dropdown with 5 options: `Idea`, `Topic`, `Decision`, `Action Item`, `Information` (values: `idea`, `topic`, `decision`, `action_item`, `information`)
- [x] `affected_disciplines[]` — required multi-select from all 15 discipline values; uses checkbox list or chip-style input; at least 1 selection required
- [x] `who` — required; text input with datalist populated from `GET /api/projects/{id}/participants` (allows free text if participant not in roster)
- [x] `date` — required native date input (`<input type="date">`), defaults to today's date
- [x] `context_notes` — optional textarea, label "Context / Notes", placeholder "Additional context, meeting reference, or source description"

### Conditional Fields — Action Item
- [x] When `item_type === 'action_item'`: show `owner` field (text input with participant datalist, required for action items)
- [x] When `item_type === 'action_item'`: show `due_date` field (native date input, optional)
- [x] When `item_type !== 'action_item'`: `owner` and `due_date` fields are hidden and not submitted

### Conditional Fields — Decision
- [x] When `item_type === 'decision'`: show `consensus` section with label "Consensus"
- [x] Consensus section renders one row per selected discipline: discipline label + status select (`Agree`, `Disagree`, `Abstain`) + optional notes input
- [x] Consensus rows update reactively when `affected_disciplines` selection changes
- [x] When `item_type !== 'decision'`: consensus section is hidden and not submitted

### Source Type & Submission Behaviour
- [x] `source_type` field is not rendered in the UI; value is hardcoded to `manual_input` in the request payload
- [x] Submit button label: "Save Item"
- [x] Form submits via `POST /api/projects/{id}/items` with full payload (see Dev Notes)
- [x] Submit button shows loading state (`disabled`, spinner icon) during API call
- [x] On success: navigate to `/projects/:id/history?highlight={newItemId}`
- [x] On API error: display inline error banner above the submit button (do not navigate away)
- [x] Manual items bypass ingestion approval — they are directly created and immediately visible

### Validation
- [x] `statement` — shows inline error "Statement is required" if empty on submit
- [x] `item_type` — shows inline error "Item type is required" if no selection on submit
- [x] `affected_disciplines` — shows inline error "Select at least one discipline" if empty on submit
- [x] `who` — shows inline error "Who is required" if empty on submit
- [x] `date` — shows inline error "Date is required" if empty on submit
- [x] All required field errors shown simultaneously on submit attempt (not one-by-one)
- [x] Error messages linked to inputs via `aria-describedby`

### Accessibility
- [x] All inputs have associated `<label>` elements
- [x] Required fields marked with `*` (visible) and `aria-required="true"`
- [x] Keyboard navigation: Tab through all visible fields, Enter triggers submit only when focus is on submit button
- [x] Discipline multi-select checkboxes are keyboard-navigable (Space to toggle, Arrow keys to move)
- [x] Consensus status selects labelled with discipline name for screen readers

### Responsive Design
- [x] Mobile (375px): single-column, full-width inputs, disciplines in 2-column checkbox grid
- [ ] Tablet (768px): two-column layout for date/who row pairs
- [x] Desktop (1024px+): form constrained to `max-w-2xl` centered
...
- `src/tests/components/ManualItemForm.test.tsx` — Vitest + RTL tests (16 tests)
- `src/hooks/useParticipants.ts` — React Query hook for participant roster

**Modified Files:**
- `src/App.tsx` — Added `/projects/:id/items/new` route + ManualItemCreate import
- `src/types/projectItem.ts` — Added `source_type`, `context_notes`, `date`, `due_date` to `ProjectItemCreate`; exported `ALL_DISCIPLINES` constant
...
| 2026-02-27 | Implemented: ManualItemForm, ManualItemCreate page, useParticipants hook, route, types update, 16 tests — all passing |
