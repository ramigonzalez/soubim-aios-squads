...
- [ ] `/ingestion` route is registered in the router
- [ ] Route renders the `IngestionApproval` organism wrapped in the `Ingestion` page component
- [ ] Non-admin users are redirected away from `/ingestion` (role check against auth store)
- [ ] Navigation link to `/ingestion` is only visible to admin users (Story 9.5)

### Meeting Source Rows
- [ ] Each meeting row displays: Call ID, Project, Date/Time, Meeting Type, Source, Include toggle, AI Summary (collapsed), Transcript link
- [ ] Transcript link opens in a new tab
- [ ] AI Summary expands/collapses inline on click (no modal)

### Email Source Rows
- [ ] Each email row displays: Email ID, Project, Date, Subject, From, To/CC count, Include toggle, AI Summary (collapsed), Thread link
- [ ] Thread link opens in a new tab
- [ ] To/CC count shown as a numeric badge (e.g. "3 recipients")
- [ ] AI Summary expands/collapses inline on click

### Document Source Rows
- [ ] Each document row displays: Document ID, Project, Upload Date, File Name, File Type, File Size, Include toggle, AI Summary (collapsed), File link
- [ ] File size displayed in human-readable format (e.g. "2.4 MB")
- [ ] File link opens/downloads in a new tab
- [ ] AI Summary expands/collapses inline on click

### Include Toggle
- [ ] Each row has an Include toggle (checkbox or switch) that immediately calls `PATCH /api/ingestion/{source_id}` to approve or reject
- [ ] Toggle reflects the current status (checked = approved, unchecked = rejected, indeterminate = pending)
- [ ] Optimistic update: toggle changes immediately; reverts on API error with toast notification

### Bulk Actions
- [ ] Checkbox on each row selects it (stored in Zustand ingestion store)
- [ ] "Select All" checkbox in header row selects/deselects all visible rows
- [ ] Bulk action bar appears when one or more rows are selected
- [ ] "Approve Selected" button calls `POST /api/ingestion/batch` with `{source_ids: [...], action: 'approve'}`
- [ ] "Reject Selected" button calls `POST /api/ingestion/batch` with `{source_ids: [...], action: 'reject'}`
- [ ] Bulk action bar shows count of selected items ("3 items selected")
- [ ] After bulk action completes, selection is cleared and row list refetches

### Filters
- [ ] Filter by project: dropdown/select populated from available projects
- [ ] Filter by source type: chips for "All", "Meeting", "Email", "Document"
...
- [ ] Filters are applied client-side against the fetched source list (no additional API call per filter change)

### Status Indicators
- [ ] Pending status: yellow badge (`bg-yellow-100 text-yellow-800`)
- [ ] Approved status: green badge (`bg-green-100 text-green-800`)
- [ ] Rejected status: red badge (`bg-red-100 text-red-800`)
- [ ] Processed status: blue badge (`bg-blue-100 text-blue-800`)
- [ ] Status badge displayed on every row

### Navigation Badge
- [ ] Navigation item for "Ingestion" shows a count badge with the number of pending items
- [ ] Badge fetches count from `GET /api/ingestion?status=pending&count_only=true` (or derives from cached list)
- [ ] Badge updates when items are approved/rejected (React Query cache invalidation)
- [ ] Badge hidden when pending count is 0

### Loading, Empty, and Error States
- [ ] Loading state: skeleton rows with `animate-pulse` (3 rows per source type)
- [ ] Empty state when no pending sources: icon + "No pending items" heading + description
- [ ] Error state: error message with "Retry" button that triggers React Query refetch

### Responsive Design
- [ ] Mobile (375px): horizontally scrollable table or stacked card layout per row
- [ ] Tablet (768px): full table layout with overflow-x-auto wrapper
- [ ] Desktop (1024px+): full table, max-w-7xl container

### Accessibility
- [ ] Table uses `<table>`, `<thead>`, `<tbody>`, `<th scope="col">`, `<td>` semantic elements
- [ ] Include toggles have associated labels with `aria-label="Include {source_id}"`
- [ ] Status badges have `aria-label="Status: {status}"`
- [ ] Bulk action bar is announced via `aria-live="polite"` when it appears/disappears
- [ ] Keyboard navigation through rows: Tab, Space to toggle checkboxes, Enter to expand AI summary
