...
- [x] All visible UI text referring to "Timeline" changed to "Project History"
- [x] Locations to update:
  - [x] Project Detail page heading/tab label
  - [x] Navigation breadcrumb
  - [x] Empty state messages
  - [x] Filter bar label/heading (if any)
  - [x] Loading state text
  - [x] Error state text
  - [x] Any aria-labels referencing "Timeline"
- [x] **Code internals NOT renamed** — component file names, hook names, and internal variables keep "Timeline" for backward compatibility (rename is UI-only)
- [x] `Milestone Timeline` label remains as-is (it is already correct)

### Tab Toggle
- [x] Project Detail page shows tab toggle: **"Milestone Timeline"** | **"Project History"** | **"Executive Digest"**
- [x] Tab styling: segmented control (consistent with GroupingToggle pattern)
  ```
  inline-flex rounded-lg border border-gray-200 p-0.5 bg-gray-100
  ```
- [x] Active tab: `bg-white rounded-md shadow-sm text-gray-900 font-medium`
- [x] Inactive tab: `text-gray-500 hover:text-gray-700`
- [x] Tab position: above filter bar, below project title/breadcrumb
- [x] Icons on tabs: `Star` for Milestone Timeline, `Clock` for Project History, `FileText` for Executive Digest

### Default View Logic
- [x] If project has stages (from Story 6.1): default to "Milestone Timeline" tab
- [x] If project has NO stages: default to "Project History" tab
- [x] Logic: defaults to 'milestones'; stages-based logic ready to wire when useStages hook is available (Story 6.1)

### URL Hash Persistence
- [x] Active tab encoded in URL hash: `#milestones`, `#history`, or `#digest`
- [x] On page load: read hash → set active tab
- [x] On tab switch: update hash without page reload (uses `history.replaceState`)
- [x] Deep linking: `/projects/123#milestones` opens Milestone Timeline tab
- [x] Deep linking: `/projects/123#history` opens Project History tab
- [x] No hash: defaults to milestones

### Breadcrumb Update
- [x] Breadcrumb pattern: `Projects > Project > {Current Tab}`
- [x] Current Tab shows: "Milestone Timeline", "Project History", or "Executive Digest"
- [x] Breadcrumb updates when switching tabs

### Filter State Isolation
- [x] Milestone Timeline and Project History have independent filter states
- [x] Switching tabs does not reset the other tab's filters
- [x] FilterBar only shows in Project History tab; Milestone Timeline has no filter bar currently
...
**New Files:**
- `src/tests/pages/ProjectDetail.test.tsx` — 19 tests covering tab toggle, URL hash, breadcrumb, rename verification

**Modified Files:**
- `src/pages/ProjectDetail.tsx` — 3-tab segmented control (milestones/history/digest), URL hash persistence, breadcrumb, dynamic heading, Star/Clock/FileText icons
- `src/components/organisms/Timeline.tsx` — Error state text updated: "Failed to load decisions" → "Failed to load project history"
- `src/components/common/Navigation.tsx` — No changes needed (no "Timeline" text found)
...
| 2026-02-28 | Implemented: tab toggle, URL hash persistence, breadcrumb, error text rename, 19 tests (all pass) |
