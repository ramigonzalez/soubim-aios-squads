...
- [ ] Google Drive API OAuth2 authentication configured (service account recommended for server-to-server)
- [ ] OAuth credentials stored securely in environment variables:
  ...
  - [ ] `GOOGLE_DRIVE_ENABLED` — boolean flag to enable/disable monitoring (default: false)
- [ ] Drive API client initialized at startup when enabled
- [ ] Scopes: `https://www.googleapis.com/auth/drive.readonly`

### Folder Configuration (Per Project)
- [ ] New field on `Project` model: `drive_folder_id: string?`
- [ ] `PATCH /api/projects/{id}` accepts `drive_folder_id` to configure monitoring
- [ ] Project Form (Story 6.2) shows optional "Google Drive Folder ID" text input
- [ ] Validation: verify folder is accessible with configured credentials on save
- [ ] Admin-only configuration

### Polling Mechanism
- [ ] Background polling job runs on a configurable schedule
- [ ] Polling frequency configurable via `DRIVE_POLL_INTERVAL_MINUTES` env variable (default: 60)
- [ ] Uses APScheduler (consistent with Gmail poller in Story 7.4)
- [ ] Job queries all projects with non-null `drive_folder_id`
- [ ] For each project folder:
  - [ ] List files modified/created since last poll timestamp
  - [ ] Filter: only `.pdf` and `.docx` files
  - [ ] Skip files already processed (deduplication by Drive file ID)

### File Processing
- [ ] For each new file discovered:
  - [ ] Download file content from Google Drive API
  - [ ] Extract text using `DocumentProcessor` (Story 10.2)
  - [ ] Generate AI one-line summary from first 2000 chars
  - [ ] Create `Source` record:
    ...
    - [ ] `drive_folder_id` = originating folder ID
    - [ ] `ai_summary` = one-line summary
  - [ ] Store file locally in `uploads/documents/{project_id}/{source_id}.{ext}`

### Deduplication
- [ ] Track processed Drive file IDs to avoid re-processing
- [ ] New column on `Source`: `drive_file_id: string? (UNIQUE where not null)`
- [ ] Before creating Source, check if `drive_file_id` already exists
- [ ] Skip silently if duplicate (log at debug level)

### Error Handling
- [ ] Drive API authentication failure: log error, disable polling for that cycle, retry next cycle
- [ ] Folder not found / access denied: log warning, skip project
- [ ] File download failure: log error, skip file, continue with others
- [ ] Text extraction failure: create Source with empty `raw_content`, mark for manual review
- [ ] Rate limiting: respect Google Drive API quotas (1000 queries/100 seconds per user)
- [ ] Network timeout: 30-second timeout per API call

### Monitoring & Logging
- [ ] Log each poll cycle: "Polling {N} project folders"
- [ ] Log each new file discovered: "Found new file: {filename} in project {project_name}"
- [ ] Log each Source created: "Created source {source_id} from Drive file {file_id}"
- [ ] Track last successful poll timestamp per project (stored in project metadata or separate table)
...
- `decision-log-backend/tests/test_drive_monitor.py`
- `decision-log-backend/tests/test_drive_client.py`

**Modified Files:**
- `decision-log-backend/app/database/models/project.py` — Add drive_folder_id, last_drive_poll
- `decision-log-backend/app/database/models/source.py` — Add drive_file_id
- `decision-log-backend/app/main.py` — Schedule polling job
- `decision-log-backend/requirements.txt` — Add google-api-python-client, google-auth
- `decision-log-frontend/src/components/organisms/ProjectForm.tsx` — Add Drive folder ID field
- `decision-log-backend/.env.example` — Document new env vars
...
### Unit Tests
```
- ✅ Drive client lists files in folder with date filter
- ✅ Drive client downloads file content
- ✅ Drive client verifies folder access
- ✅ Monitor creates Source records for new files
- ✅ Monitor skips duplicate files (by drive_file_id)
- ✅ Monitor only processes PDF/DOCX files
- ✅ Monitor updates last_drive_poll timestamp
- ✅ Monitor handles inaccessible folder gracefully
- ✅ Monitor handles download failure gracefully
- ✅ Polling job disabled when GOOGLE_DRIVE_ENABLED=false
```
