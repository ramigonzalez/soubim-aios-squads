...
- [ ] `decision-log-backend/app/prompts/` directory created
- [ ] `extract_meeting.md` prompt file created (Phase 1 — meetings)
- [ ] `extract_email.md` prompt file created (Phase 4 placeholder — email extraction)
- [ ] `extract_document.md` prompt file created (Phase 4 placeholder — document extraction)
- [ ] Prompt loader service (`prompt_loader.py`) loads prompts from disk at runtime
- [ ] Prompts NOT hardcoded in Python — externalized for easy iteration

### Extraction Service Update
- [ ] Extraction service refactored to call Claude 3.5 Sonnet with loaded prompts
- [ ] Service sends `ProjectParticipant[]` roster as context in every extraction call
- [ ] Service receives structured JSON output with multiple items of different types
- [ ] Each extracted item includes: `item_type`, `statement`, `who`, `timestamp`, `affected_disciplines[]`, `context`
- [ ] Type-specific fields per "Structured Body per Item Type" table (PRD):
  - [ ] **idea**: `context`, `related_topic` (optional)
  - [ ] **topic**: `discussion_points`, `status_note`, `next_steps` (optional)
  - [ ] **decision**: `why`, `impacts`, `causation`, `consensus` (required for consensus)
  - [ ] **action_item**: `owner` (required), `due_date`, `is_done` (defaults to false)
  - [ ] **information**: `reference_source`, `date_of_fact` (optional)

### Discipline Inference
- [ ] Extraction prompt receives `ProjectParticipant[]` as context (name, email, discipline)
- [ ] Discipline inference logic per item type (as per PRD guidelines):
  - [ ] **decision**: disciplines with `AGREE` status in consensus
  - [ ] **topic**: all disciplines actively participating in discussion
  - [ ] **idea**: discipline of proposer + explicitly mentioned disciplines
  - [ ] **action_item**: discipline of owner + disciplines the deliverable impacts
  - [ ] **information**: disciplines explicitly referenced or impacted
- [ ] `affected_disciplines[]` array populated for each item

### Storage & Pipeline Integration
- [ ] Extraction results stored as `project_items` records (not `decisions`)
- [ ] `item_type` field populated correctly for each extracted item
- [ ] Action items include `owner` field when mentioned in transcript
- [ ] Action items default `is_done=false` on creation
- [ ] Agent enrichment pipeline (vector embeddings, semantic clustering) adapted to work with all 5 item types

### Prompt Accuracy & Testing
- [ ] Test suite with 10 curated transcript snippets created (covering all 5 item types)
- [ ] Extraction achieves **99%+ accuracy** on item type classification across test set
- [ ] Test cases verify: item type classification, affected_disciplines inference, type-specific field extraction
- [ ] Regression test: re-processing an existing V1 transcript produces same decisions plus new item types
- [ ] All extracted items generate vector embeddings correctly
