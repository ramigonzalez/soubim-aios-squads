...
### Meeting Summary
- [ ] `FileText` icon button appears in meeting `SourceGroupAccordion` header
- [ ] Icon visible on hover: `opacity-0 group-hover:opacity-100 transition-opacity`
- [ ] Click expands a summary section below the header, above item rows
- [ ] Summary displays AI-generated text from `Source.ai_summary` field
- [ ] Summary section styling:
  - [ ] Background: `bg-blue-50/50`
  - [ ] Padding: `px-4 py-3`
  - [ ] Text: `text-sm text-gray-700 italic`
  - [ ] Border: `border-b border-blue-100`
- [ ] Collapse toggle: click icon again to hide summary
- [ ] Summary state is local (component-level, not in store)
- [ ] If no `ai_summary` available, icon is hidden
- [ ] Summary section animates open/close: `transition-all duration-200`

### Source Type Filters
- [ ] New filter group: "Source" with 4 toggle chips
- [ ] Chips follow UX Guidelines Section 2.8 colors:
  - [ ] Meeting: `Video` icon, active: `bg-indigo-100 text-indigo-700`
...
- [ ] Toggle behavior: click to activate/deactivate (multi-select)
- [ ] Inactive style: `bg-gray-100 text-gray-600`
- [ ] Item count badge on each chip showing how many items from that source

### Item Type Filters
- [ ] New filter group: "Type" with 5 toggle chips
- [ ] Chips follow UX Guidelines Section 2.7 colors:
...
- [ ] Same toggle behavior and styling pattern as source chips

### Filter Logic
- [ ] **AND across dimensions:** source + item type + discipline + date + search all combine
- [ ] **OR within each dimension:** e.g., "Meeting" + "Email" shows items from either
- [ ] Filters applied client-side to already-fetched data
- [ ] Empty result: "No items match your filters." + "Clear filters" link
- [ ] Filter response time: <200ms (client-side)
- [ ] Debounce search input: 300ms

### Filter Bar Layout
- [ ] Source and Item Type chips render in the existing FilterBar component
- [ ] Layout: Search row → Filter chips row (Discipline | Source | Type | Date | Who) → Active chips row
- [ ] Each filter group visually separated with a thin divider or label
- [ ] "Clear all" button resets all filters including new ones

### Filter State Persistence
- [ ] New filter dimensions added to `filterStore.ts`:
  ```typescript
  ...
  ```
- [ ] URL param encoding: `?source=meeting,email&type=decision,topic&discipline=architecture`
- [ ] On page load: URL params → filterStore hydration
- [ ] On filter change: filterStore → URL params update (via `replaceState`)
- [ ] Shareable: copy URL → open → same filters applied

### Item Count Badges
- [ ] Each filter chip shows count of matching items: `(12)` badge
- [ ] Count updates reactively as other filters are applied
- [ ] Badge styling: `text-[10px] font-medium text-gray-400 ml-0.5`
...
- `src/tests/hooks/useFilterUrlSync.test.tsx`

**Modified Files:**
- `src/store/filterStore.ts` — Add sourceTypes, itemTypes state + actions
...
- `src/tests/components/FilterBar.test.tsx` — Updated for new filter dimensions
...
### Unit Tests
```
- ✅ MeetingSummary renders and hides correctly
- ✅ Source type chips toggle on/off
- ✅ Item type chips toggle on/off
- ✅ Combined filter logic: source + type + discipline + date + search
- ✅ OR within dimension, AND across dimensions
- ✅ Count badges show correct totals
- ✅ URL params hydrate filters on mount
- ✅ Filter changes update URL params
- ✅ Clear all resets all filter dimensions
- ✅ Empty result state shows message
- ✅ Meeting summary icon hidden when no ai_summary
```
